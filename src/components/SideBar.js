...
export default function SideBar({ weatherData, setLocation, celsius }) {
  ...
  const fetchData = () => {
    return fetch(
      `http://api.weatherapi.com/v1/search.json?key=4bf7f3ea42fc436fbe145440230203 &q=${searchValue}`
    )
            ...
            {showCityList ? (
              <ul className="absolute top-11 left-0 w-full bg-[#6E707A] ">
                {cityList.length > 0 && cityList !== "undefined"
                  ? Object.keys(cityList)?.map((key, i) => {
                      return (
                        <li
                          key={i}
                          className="text-white block border-b text-left last:border-b-0 hover:bg-[#100E1D] transition-all border-white p-1"
                        >
                          <button
                            onClick={locationHandler}
                            className="text-left block w-full"
                            value={cityList[key]?.name}
                          >
                            {cityList[key]?.name}
                          </button>
                        </li>
          ...
          <h3 className="text-white font-medium text-center text-8xl sm:text-[161px]">
            {celsius === true ? (
              <span>
                ...
                <sub className="text-5xl text-[#A09FB1]">℃</sub>
              </span>
            ) : (
              <span>
                {Math.floor(weatherData?.current?.temp_f)}
                <sub className="text-5xl text-[#A09FB1]">°F</sub>
              </span>
            )}
          </h3>
        </FullCol>
        <FullCol className="text-5xl pt-8 text-[#A09FB1] text-center">
          <h5>{weatherData?.current?.condition?.text}</h5>
        </FullCol>
        <FullCol>
          <FullCol className="pt-10 ">
            <p className="text-[#A09FB1] text-base text-center">
              {new Date().getDate() < 10 ? (
                <> 0{new Date().getDate()}</>
              ) : (
                new Date().getDate()
